...
//     useEffect(() => {
//   if (product?.image[0]) {
//     setThumbnail(`${API_URL}/images/${product.image[0]}`);
//   } else {
//     setThumbnail(null);
//   }
// }, [product, API_URL]);
    useEffect(() => {
  if (product?.image?.[0]) {
    setThumbnail(product.image[0].url);
  } else {
    setThumbnail(null);
  }
}, [product]);
return product && (
  ...
  <div
    key={index}
    onClick={() => setThumbnail(imgObj.url)}
  ...
  >
    <img src={imgObj.url} alt={`Thumbnail ${index + 1}`} />
        ...
        <div className="border border-gray-500/30 max-w-100 rounded overflow-hidden">
          <img
  src={thumbnail || product.image?.[0]?.url || '/placeholder.png'}
